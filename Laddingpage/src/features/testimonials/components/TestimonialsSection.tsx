...
import type { Testimonial as ApiTestimonial } from '../../api/types';
...
export default function TestimonialsSection({
  ...
  const mappedTestimonials = useMemo(() => {
    if (apiTestimonials.length > 0) {
      return apiTestimonials.map(mapApiTestimonialToComponent);
    }
    return defaultTestimonialsParam || defaultTestimonials || [];
  }, [apiTestimonials, defaultTestimonialsParam]);

  const testimonials = mappedTestimonials;
  ...
  if (error) {
    return (
      <section className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-white via-pink-50/40 to-white">
  ...
  return (
    <section
      id="testimonials"
      className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-white via-pink-50/40 to-white"
    >
      <div className="container mx-auto max-w-7xl">
        {/* Section Header */}
